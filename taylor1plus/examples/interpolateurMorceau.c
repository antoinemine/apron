...
//    ap_abstract1_fprint(stdout, man, &abs);
    ...
    ap_lincons1_array_t cons_assert = ap_lincons1_array_make(env, 2);
    ap_lincons1_array_set(&cons_assert, 0, &cons_ymun);
    ap_lincons1_array_set(&cons_assert, 1, &cons_ymun);


    ap_abstract1_t abs1, abs2, abs3, abs4, abs5, abs6, abs7;
    abs1 = ap_abstract1_meet_lincons_array(man, false, &abs, &cons_if);
    abs2 = ap_abstract1_assign_linexpr(man, false, &abs1, "y", &dixx, NULL);
    abs3 = ap_abstract1_meet_lincons_array(man, false, &abs, &cons_else);
    abs4 = ap_abstract1_assign_linexpr(man, false, &abs3, "y", &vinx, NULL);
    ...
    abs7 = ap_abstract1_meet_lincons_array(man, false, &abs6, &cons_assert);

    ap_abstract1_fprint(stdout, man, &abs7);
    ap_box1_t gamma = ap_abstract1_to_box(man, &abs7);
