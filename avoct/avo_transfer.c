...
/* set *exact to 1 if all all constraints are avoagonal ones
  ...
  *exact = 1;
  for (i=0;i<ar->size;i++) {
	...
	if (avo_hmat_close_incremental(b,nsc,dim,var_pending)) return true;
      ...
      bound_bmin(b[avo_matpos(ui,ui^1)],pr->tmp[1]);
      /*  c_i X_i + [-a,b] >= 0 <=> -c_i X_i <= b */
      if (c==AP_CONS_EQ) bound_bmin(b[avo_matpos(ui^1,ui)],pr->tmp[0]);
      /*  c_i X_i + [-a,b] <= 0 <=>  c_i X_i <= a */
      if (c==AP_CONS_SUP) {
    	  *exact = 0;
    	  if(bound_cmp(b[avo_matpos(ui,ui^1)],pr->tmp[1])==0)
    	     bound_set_int(nsc[avo_matpos(ui,ui^1)],0);
      }
      break;
		  /* not exact for strict constraints */
     case BINARY:
      /* can we delay incremental closure further? */
	...
	if (avo_hmat_close_incremental(b,nsc,dim,var_pending)) return true;
      ...
      bound_bmin(b[avo_matpos2(uj,ui^1)],pr->tmp[1]);
      /*  c_i X_i + c_j X_j + [-a,b] >= 0 <=> -c_i X_i - c_j X_j <= b */
      if (c==AP_CONS_EQ) bound_bmin(b[avo_matpos2(uj^1,ui)],pr->tmp[0]);
      /*  c_i X_i + c_j X_j + [-a,b] <= 0 <=>  c_i X_i + c_j X_j <= a */
      if (c==AP_CONS_SUP)
      {
    	  *exact = 0;
    	  if(bound_cmp(b[avo_matpos2(uj,ui^1)],pr->tmp[1])==0)
			      ...
			      ap_lincons0_array_t* array)
{
  ...
  avo_internal_t* pr =
    avo_init_from_manager(man,AP_FUNID_MEET_LINCONS_ARRAY,4*(a->dim+8));
  if (!a->closed && !a->m)
    /* definitively empty */
    return avo_set_mat(pr,a,NULL,NULL,destructive);
  else {
    ...
    if (avo_hmat_add_lincons(pr,m,m1,a->dim,array,&exact,&respect_closure)) {
      ...
      /* exact if avoagonal constraints & no conversion error */
...
//	fprintf(stdout, "AP_FUNID_ADD_RAY_ARRAY\n"); fflush(stdout);
  ...
  avo_internal_t* pr =
    avo_init_from_manager(man,AP_FUNID_ADD_RAY_ARRAY,2*(a->dim+2));
  bound_t *m;
  ...
  if (!m)
    ...
    return avo_set_mat(pr,a,m,NULL,destructive);
  }
}
			...
			size_t d, bool* respect_closure)
{
  ...
  {
   bound_set_infty(m[avo_matpos2(2*d+2*dim,ljc)],1);
   bound_set_infty(m[avo_matpos2(2*d+2*dim+1,ljc)],1);
   bound_set_infty(nsc[avo_matpos2(2*d+2*dim,ljc)],1);
   bound_set_infty(nsc[avo_matpos2(2*d+2*dim+1,ljc)],1);
   bound_set_infty(m[avo_matpos2(ljc,2*d+2*dim)],1);
   bound_set_infty(m[avo_matpos2(ljc,2*d+2*dim+1)],1);
   bound_set_infty(nsc[avo_matpos2(ljc,2*d+2*dim)],1);
   bound_set_infty(nsc[avo_matpos2(ljc,2*d+2*dim+1)],1);
  }
   if (u.type==ZERO ) {
...
avo_t* org_avo_assign_linexpr(ap_manager_t* man,
			  bool destructive, avo_t* a,
			  ap_dim_t d, ap_linexpr0_t* expr,
			  avo_t* dest)
{
//	fprintf(stdout, "AP_FUNID_ASSIGN_LINEXPR\n"); fflush(stdout);

  avo_internal_t* pr =
  avo_init_from_manager(man,AP_FUNID_ASSIGN_LINEXPR_ARRAY,2*(a->dim+1+5));
  ...
  m = a->closed ? a->closed : a->m;
  m1 = a->nsc;
  if (!m) return avo_set_mat_nsc(pr,a,NULL,NULL,NULL,destructive); /* empty */
  ...
  if (!destructive)
	  {
	  m = avo_hmat_copy(pr,m,a->dim);
	  m1 = avo_hmat_copy(pr,m1,a->dim);
	  }

  /* go */
  avo_hmat_assign(pr,u,m,m1,a->dim,d,&respect_closure);
			  ...
			  avo_t* dest)
{
	  ...
	  {
		  avo_t *re = org_avo_assign_linexpr(man,destructive,a,d,expr,dest);
		  return re;
	  }
	  else
	  {
		  ...
		  a1 = org_avo_assign_linexpr(man,true,a1,d,expr,NULL);
		  a2 = org_avo_assign_linexpr(man,true,a2,d,expr,NULL);
...
//	fprintf(stdout, "AP_FUNID_SUBSTITUTE_LINEXPR_ARRAY\n"); fflush(stdout);
  ...
  avo_internal_t* pr =
    avo_init_from_manager(man,AP_FUNID_SUBSTITUTE_LINEXPR_ARRAY,2*(a->dim+1+5));
  uexpr u = avo_uexpr_of_linexpr(pr,pr->tmp,expr,a->dim);
  bound_t * m, *m2;
  bool respect_closure;
  arg_assert(d<a->dim,return NULL;);
  ...
  if ((u.type!=UNARY || u.i!=d) && pr->funopt->algorithm>=0)
    avo_cache_closure(pr,a);
  m = a->closed ? a->closed : a->m;
  if (!m) return avo_set_mat(pr,a,NULL,NULL,destructive); /* empty */
  ...
  /* go */
  if (avo_hmat_subst(pr,u,m,a->nsc,a->dim,d,(bound_t*)m2,&respect_closure)) {
    /* empty */
    if (!destructive) avo_hmat_free(pr,m,a->dim);
    return avo_set_mat(pr,a,NULL,NULL,destructive);
  }
  ...
  if (respect_closure) return avo_set_mat(pr,a,NULL,m,destructive);
  else return avo_set_mat(pr,a,m,NULL,destructive);
}
				...
				avo_t* dest)
{
  ...
  avo_internal_t* pr =
    avo_init_from_manager(man,AP_FUNID_ASSIGN_LINEXPR_ARRAY,2*(a->dim+size+5));
  ...
  mm = avo_hmat_alloc_top(pr,a->dim+size);
  mm1 = avo_hmat_alloc_top(pr,a->dim+size);
  bound_set_array(mm,m,avo_matsize(a->dim));
  bound_set_array(mm1,m1,avo_matsize(a->dim));
  ...
  for (i=0;i<size;i++) {
    ...
    uexpr u = avo_uexpr_of_linexpr(pr,pr->tmp,texpr[i],a->dim);

    if (u.type==EMPTY) {
      avo_hmat_free(pr,mm,a->dim+size);
      avo_hmat_free(pr,mm1,a->dim+size);
      return avo_set_mat_nsc(pr,a,NULL,NULL,NULL,destructive);
    }

    if (u.type==BINARY || u.type==OTHER) inexact = 1;

    avo_hmat_assign(pr,u,mm,mm1,a->dim+size,a->dim+i,&respect_closure);
  }

  /* now close & remove temporary variables */
  if (pr->funopt->algorithm>=0) {
    if (avo_hmat_close(mm,mm1,a->dim+size)) {
      /* empty */
      avo_hmat_free(pr,mm,a->dim+size);
      avo_hmat_free(pr,mm1,a->dim+size);
      return avo_set_mat_nsc(pr,a,NULL,NULL,NULL,destructive);
  ...
  if (!destructive)
	  {
	  m = avo_hmat_alloc(pr,a->dim);
	  m1 = avo_hmat_alloc(pr,a->dim);
	  }
  ...
  avo_hmat_permute(m,mm,a->dim,a->dim+size,d);
  avo_hmat_permute(m1,mm1,a->dim,a->dim+size,d);
  avo_hmat_free(pr,mm,a->dim+size);
  avo_hmat_free(pr,mm1,a->dim+size);
...
//	  fprintf(stdout, "AP_FUNID_SUBSTITUTE_LINEXPR_ARRAY\n"); fflush(stdout);
  ...
  for (i=0;i<size;i++) {
    ...
    avo_hmat_forget_var(mm,a->dim+size,tdim[i]);
  }
  ...
  for (i=0;i<size;i++) {
    ...
    if (avo_hmat_subst(pr,u,mm,a->nsc,a->dim+size,a->dim+i,m2,
		   &respect_closure)) {
      /* empty */
      avo_hmat_free(pr,mm,a->dim+size);
      return avo_set_mat(pr,a,NULL,NULL,destructive);
  ...
  /* now close */
  if (pr->funopt->algorithm>=0) {
    if (avo_hmat_close(mm,a->nsc,a->dim+size)) {
      /* empty */
      avo_hmat_free(pr,mm,a->dim+size);
      return avo_set_mat(pr,a,NULL,NULL,destructive);
    }
  ...
  /* remove temp */
  if (!destructive) m = avo_hmat_copy(pr,mm,a->dim);
  else bound_set_array(m,mm,avo_matsize(a->dim));
  avo_hmat_free(pr,mm,a->dim+size);
