...
void sqrt_householder(ap_manager_t* man, double init, int *N, double *inf, double *sup, FILE* stream)
{
	...
	/* hn = 1 - A*xn*xn */
	ap_texpr1_t* hn = ap_texpr1_binop(AP_TEXPR_SUB,
			one,
			...
			AP_RDIR_UP);
	/* xnp1 = */
	ap_texpr1_t* xnp1_scheme = ap_texpr1_binop(AP_TEXPR_ADD,
			ap_texpr1_var(env, "xn"),
			ap_texpr1_binop(AP_TEXPR_MUL,
				ap_texpr1_var(env, "xn"),
				ap_texpr1_binop(AP_TEXPR_ADD,
					ap_texpr1_binop(AP_TEXPR_MUL,
						ap_texpr1_var(env, "hn"),
						half,
						AP_RTYPE_DOUBLE,
						AP_RDIR_UP),
					ap_texpr1_binop(AP_TEXPR_MUL, 
	...
	ap_texpr1_t* zz = ap_texpr1_binop(AP_TEXPR_MUL,
			ap_texpr1_var(env, "z"),
			ap_texpr1_var(env, "z"),
			AP_RTYPE_DOUBLE,
			AP_RDIR_UP);
	...
	pivot = ap_abstract1_copy(man, &_19);
	...
	while (i<INITIAL_UNFOLD) 
	{
		...
		_15 = ap_abstract1_assign_texpr(man, false, &_14, "z", z, NULL);
