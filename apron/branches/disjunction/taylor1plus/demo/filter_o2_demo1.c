...
*/
	...
	ap_box1_t box;
	ap_abstract1_t _15,_16,_17,_18,_19,_20,_21,pivot,_k1,_k2, pivot2, fix, init;
	...
	if (T1P)
	{
		...
		array[0] = ap_abstract1_copy(man, &pivot);
		i = 0;
		while (i<(THRESHOLD-1)) 
		{
			fprintf(stream, "/*** X%d ***/\n",i);
			ap_abstract1_fdump(stream, man, &array[i]);
			fprintf(stream,"\n");
		...
		init = ap_abstract1_join_array(man, array, THRESHOLD);
		fprintf(stream,"/*** Y0 = X0 U F(X0) U ... U F^%d(X0) ***/\n",i);
		ap_abstract1_fdump(stream, man, &init);
		fprintf(stream,"\n");
		...
		ap_abstract1_clear(man, &pivot);
		pivot = ap_abstract1_copy(man, &init);
		ap_abstract1_clear(man, &init);
	}
	while (LOOP) {
		...
		if (ap_abstract1_is_leq(man, &pivot, &_k1) == tbool_true) {
			/* Compute F(X) */
			_15 = ap_abstract1_assign_linexpr(man, false, &_k1, "E1", &E0, NULL);
			_16 = ap_abstract1_assign_linexpr(man, false, &_15, "E0", &E, NULL);	
			_17 = ap_abstract1_assign_linexpr(man, false, &_16, "E", &top, NULL);
			_18 = ap_abstract1_meet_lincons_array(man, false, &_17, &array_consE);
			_19 = ap_abstract1_assign_linexpr(man, false, &_18, "S1", &S0, NULL);
			_20 = ap_abstract1_assign_linexpr(man, false, &_19, "S0", &S, NULL);
			_21 = ap_abstract1_assign_linexpr(man, false, &_20, "S", &h, NULL);
			if (ap_abstract1_is_leq(man, &_21, &_k1) == tbool_true) {
				printf("F(X) <= X\n");
			} else {
				printf("F(X) !<= X\n");
				box = ap_abstract1_to_box(man, &_21);
				ap_double_set_scalar(inf, box.p[3]->inf, GMP_RNDU);
				ap_double_set_scalar(sup, box.p[3]->sup, GMP_RNDU);
				printf("T1P: F(xn): [%.2f, %.2f]\n",*inf, *sup);
				ap_box1_clear(&box);
				box = ap_abstract1_to_box(man, &_k1);
				ap_double_set_scalar(inf, box.p[3]->inf, GMP_RNDU);
				ap_double_set_scalar(sup, box.p[3]->sup, GMP_RNDU);
				printf("T1P: xn: [%.2f, %.2f]\n",*inf, *sup);
				ap_box1_clear(&box);
			...
			*N = j;
			...
			if (wide) {
				...
				_k2 = ap_abstract1_join(man, false, &_k1, &pivot);
				j++;
				fprintf(stream,"/*** Y%d ***/\n",j);
			}
			if (j>=WIDENING_THRESHOLD && WIDENING_THRESHOLD>0) wide = 1;
...
int main(void)
{
	...
	start = clock();
	filter1(man, &N, &inf, &sup, stream, &cpu_time_used);
	end = clock();
