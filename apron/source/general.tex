...
%----------------------------------------------------------------------
\point{Gestionnaire.} 
Un contexte d'appel, objet de type {\Tt{}manager{\_}t*}, sera explicitement passé à
...
{\Tt{}setjmp}), et celui-ci étant spécifiques à chaque langage (C++,
OCaml, Java), les informations d'exception sont retournées
uniquement via la structure {\Tt{}manager{\_}t}.
...
Lancement d'une exception {\Tt{}not{\_}implemented} lorsqu'une fonction
...
\item Le mode destructif permet d'implémenter un mode effet de bord comme
  suit: {\Tt{}a\ =\ meet(man,destructive=true,a,b);}: on peut
  considérer que l'on fait un effet de brord sur {\Tt{}a}.
...
Le système {\Tt{}varargs} ne sera supporté que si un des participants
en a le beosin (aucun pour l'instant). \bj{La version
{\Tt{}vararg} est implantable en utilisant la version tableau, donc
...
\nwenddocs{}\nwbegincode{1}\moddef{bidon}\endmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
...
{\Tt{}sizeof(object{\_}t)} est sensiblement plus grand que la taille
d'un scalaire (4 ou 8 octets). Inconvénient: casse-bonbon pour le
programmeur de devoir écrire {\Tt{}toto({\&}x)} au lieu de {\Tt{}toto(x)}.

\item choix similaire pour la valeur retournée:
\nwenddocs{}\nwbegincode{3}\moddef{bidon}\plusendmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
...
\emph{et non un passage par référence sur les types {\Tt{}manager{\_}t}
et {\Tt{}abstract{\_}t}}. C'est d'une certaine manière une question
d'interprétation, bien sûr, mais en pratique si tous les objets
sont déclarés, construits, manipulés avec un type {\Tt{}object{\_}t*},
...
Maintenant, si le type {\Tt{}lincons{\_}t} des contraintes linéaires
utilisé dans {\Tt{}abstract0.h} est abstrait (généré à partir d'un
...
\nwendcode{}\nwbegindocs{8}\nwdocspar
\noindent
Une contrainte linéaire étant de type {\Tt{}lincons{\_}t*}, les tableaux
de contraintes linéaires sont de type {\Tt{}lincons{\_}t**}.

Mais si le type {\Tt{}lincons{\_}t} utilisés dans ces fonctions est
...
\item Les types des objets ({\Tt{}abstract{\_}t*,\ linexpr{\_}t*,\ ray{\_}t*}) 
  sont en général des types pointeurs, sauf pour:
  \begin{itemize}
  \item Les nombres ({\Tt{}coeff{\_}t});
  \item Les intervalles ({\Tt{}interval{\_}t}),
  \item Les contraintes ({\Tt{}lincons{\_}t})
  \end{itemize}
  Ces trois derniers types sont considérés comme des types
  utilisateurs \cf\sref{chap:coeff}, et leur taille (en terme de
  {\Tt{}sizeof()}) reste petite (mais ces structures peuvent bien sûr
  contenir des pointeurs vers des zones de taille importante, \eg
  nombres multi-précision).
\item On utilisera les types {\Tt{}size{\_}t} (défini dans {\Tt{}stddef.h})
  et {\Tt{}dim{\_}t} (défini par {\Tt{}unsigned\ int}) pour typer les
...
Essaye-t-on d'unifier l'interface ? En C, lorsque {\Tt{}toto(x)} 
doit retourner deux arguments {\Tt{}a} et {\Tt{}b}, on a 3 solutions;
...
\bj{Technique objet: on peut inclure dans la structure
  {\Tt{}manager{\_}t} des pointeurs sur les fonctions offertes par
  l'interface, et définir des fonctions génériques qui utilisent
  {\Tt{}manager{\_}t} pour appeler la fonction effective. C'est ce qui
