...
\nwenddocs{}\nwbegincode{1}\moddef{coeff.h}\endmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
/* ************************************************************************* */
...
des fonctions d'initialisation et de nettoyage {\Tt{}X{\_}t*\ X{\_}alloc()}
et {\Tt{}void\ X{\_}free(X{\_}t*)}.

%----------------------------------------------------------------------
\point{Représentation des nombres dans les librairies
sous-jacentes et types abstraits.} Elle est entièrement libre.
\begin{itemize}
\item Les entiers peuvent être représenté par des {\Tt{}int}, {\Tt{}long\ int},
  {\Tt{}long\ long\ int}, ou des entiers multi-précisions (d'une
  librairie quelconque).
\item les réels peuvent être représentés par des rationnels sur
  les entiers précédents, ou par des flottants ({\Tt{}float},
  {\Tt{}double} ou flottants multi-précisions), ou même des
...
représentable de manière exacte par un {\Tt{}float}.
...
La solution retenue:
\nwenddocs{}\nwbegincode{5}\moddef{coeff.h}\plusendmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
...
\item Les rationnels multi-précisions incluent le cas des
  rationnels sur des entiers à précision fixe ({\Tt{}int},
  {\Tt{}long\ int}, etc).
\item {\Tt{}double} inclut {\Tt{}float}. Toutefois, pour plus de
  généralité, on pourrait même prendre des flottants
  multi-précisions. Antoine Miné autorise {\Tt{}mpfr} dans ses
...
\item On autorise ici que chaque scalaire apparaissant dans une
  ...
  de type {\Tt{}scalar{\_}discr{\_}t} serait alors associé à
...
\nwenddocs{}\nwbegincode{7}\moddef{coeff.h}\plusendmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
...
\nwenddocs{}\nwbegincode{9}\moddef{coeff.h}\plusendmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
...
\nwenddocs{}\nwbegincode{11}\moddef{coeff.h}\plusendmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
...
\nwenddocs{}\nwbegincode{13}\moddef{coeff.h}\plusendmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
#endif
\nwendcode{}\nwbegindocs{14}\nwdocspar
\nwenddocs{}
