/* ************************************************************************* */
/* environment.c: binding of addresses (strings) to dimensions */
/* ************************************************************************* */

#include <string.h>
#include <limits.h>
#include "manager.h"
#include "abstract0.h"
#include "permutation.h"
#include "environment.h"

static inline char* strdup(const char* s){
  char* s2;

  s2 = malloc(strlen(s)+1);
  strcpy(s2,s);
  return s2;
}

/* ************************************************************************* */
/* 0. environment_var_t */
/* ************************************************************************* */

static int var_compare_default(void* v1, void* v2){
  return (v1==v2) ? 0 : strcmp((char*)v1,(char*)v2);
}
static void* var_copy_default(void* v){
  return strdup((char*)v);
}
static void var_free_default(void* v){
  free(v);
}
static char* var_to_string_default(void* v){
  return strdup((char*)v);
}
static void var_fprint_default(FILE* stream, void* v){
  fprintf(stream,"%s",(char*)v);
}

struct environment_var_t environment_var_default = {
  var_compare_default,
  var_copy_default,
  var_free_default,
  var_to_string_default,
  var_fprint_default
};

environment_var_t* environment_var = &environment_var_default;


static int var_cmp(const void* a, const void* b)
{
  void* aa = *((void**)a);
  void* bb = *((void**)b);
  return environment_var->compare(aa,bb);
}

typedef struct vardim_t {
  var_t var;
  dim_t dim;
} vardim_t;

static int vardim_cmp(const void* a, const void* b)
{
  vardim_t* aa = (vardim_t*)a;
  vardim_t* bb = (vardim_t*)b;
  return environment_var->compare(aa->var,bb->var);
}


/* ************************************************************************* */
/* I. env_t */
/* ************************************************************************* */

/* The env_t internal type store the binding name to dimension for one type.
   We use this type to factorize most operations on environements.
*/

/* ========================================================================= */
/* Datatype */
/* ========================================================================= */

typedef struct env_t {
  var_t* var_of_dim;
  /*
    Array of size size, indexed by dimensions,
    representing the map dim -> name.
    - It should not contain identical strings.
    - It is lexicographically sorted.
    - The memory allocated for the strings are attached to the structure
      (they are freed when the structure is no longer in use)
  */
  size_t size; /* Size of the array */
} env_t;

/* Free all the memory pointed from env and reinitialize fields with 0. */
void env_clear(env_t* env)
{
  size_t i;
  for (i=0;i<env->size; i++){
    environment_var->free(env->var_of_dim[i]);
    env->var_of_dim[i] = NULL;
  }
  if (env->var_of_dim){
    free(env->var_of_dim);
    env->var_of_dim = NULL;
  }
  env->size = 0;
}

/* Normalize env by sorting the array of names.
   If perm!=NULL, fill already allocated and initialized perm
   with the applied permutation*/
void env_normalize(env_t* env, dim_t* perm)
{
  if (perm==NULL){
    qsort(env->var_of_dim,env->size,sizeof(var_t),var_cmp);
  }
  else {
    size_t i;
    vardim_t* vardim = malloc(env->size*sizeof(vardim_t));
    for (i=0; i<env->size; i++){
      vardim[i].var = env->var_of_dim[i];
      vardim[i].dim = perm[i];
    }
    qsort(vardim,env->size,sizeof(vardim_t),vardim_cmp);
    for(i=0; i<env->size; i++){
      env->var_of_dim[i] = vardim[i].var;
      perm[i] = vardim[i].dim;
    }
    free(vardim);
  }
}

/* Build an object env_t from its component */
env_t env_cons(var_t* var_of_dim, size_t size)
{
  env_t res;
  res.var_of_dim = var_of_dim;
  res.size = size;
  return res;
}

/* Return true iff invariant not satisfied */
bool env_check(const env_t* env)
{
  int i;
  /* Check that there is no doublons */
  for (i=0; i<(int)env->size - 1; i++){
    if (environment_var->compare(env->var_of_dim[i],env->var_of_dim[i+1])>=0){
      return true;
    }
  }
  return false;
}

/* ========================================================================= */
/* Tests */
/* ========================================================================= */

/* Return true iff the 2 environements share some variables */
bool env_is_inter_empty(const env_t* env1 ,const env_t* env2)
{
  size_t i1,i2;
  i1 = i2 = 0;
  while (i1<env1->size && i2<env2->size){
    int sgn = environment_var->compare(env1->var_of_dim[i1],env2->var_of_dim[i2]);
    if (sgn==0)
      return false;
    if (sgn>0) i2++; else i1++;
  }
  return true;
}
/* Return true iff the 2 correct environments are equal */
bool env_is_eq(const env_t* env1,
	       const env_t* env2)
{
  size_t i;
  if (env1->size!=env2->size)
    return false;
  for (i=0; i<env1->size; i++){
    if (environment_var->compare(env1->var_of_dim[i],env2->var_of_dim[i])){
      return false;
    }
  }
  return true;
}

/* Return true iff env1 is a subset of env2 */
bool env_is_leq(const env_t* env1,
		const env_t* env2)
{
  size_t i,j;
  int sgn = 0;
  if (env1->size<=env2->size){
    j = 0;
    for (i=0;i<env1->size; i++){
      while (j<env2->size){
	sgn = environment_var->compare(env1->var_of_dim[i],env2->var_of_dim[j]);
	j++;
	if (sgn<0) return false;
	if (sgn==0) break;
      }
      if(sgn!=0) return false;
    }
    return true;
  }
  else
    return false;
}


/* ========================================================================= */
/* Operations */
/* ========================================================================= */

/* Add an array of new names to the environement and return the resulting
   environement.

   - The new names are duplicated, so it is the responsability of the user to
     free the new names he provides.

   - If a new name is already present in the initial environement, the result
     does not satisfy the datatype invariant.

   - If perm!=NULL, fill already allocated and initialized perm with the
     applied permutation to move the new names from the end to their sorted
     location.
*/
env_t env_add(const env_t* env, var_t* var_of_dim, size_t size, dim_t* perm)
{
  env_t nenv;
  size_t i;
  assert( var_of_dim!=NULL || size==0);
  nenv.var_of_dim = malloc((env->size+size)*sizeof(var_t));
  nenv.size = env->size+size;
  for (i=0;i<env->size; i++){
    nenv.var_of_dim[i] = environment_var->copy(env->var_of_dim[i]);
  }
  for (i=0;i<size; i++){
    nenv.var_of_dim[env->size+i] = environment_var->copy(var_of_dim[i]);
  }
  env_normalize(&nenv,perm);
  return nenv;
}
/* Build a environement from an array of names.
   Same comments as for env_add. */
env_t env_make(var_t* var_of_dim, size_t size)
{
  const env_t env = { NULL, 0 };
  return env_add(&env, var_of_dim, size);
}
/* Remove from an environement an array of names.

   - If a name to be removed was not present, return { NULL, UINT_MAX }.
*/
env_t env_remove(const env_t* env, var_t* var_of_dim, size_t size)
{
  env_t nenv;
  size_t i,j;

  assert( var_of_dim!=NULL || size==0);
  if (size > env->size){
    return env_cons(NULL,UINT_MAX);
  }
  nenv.var_of_dim = malloc((env->size-size)*sizeof(var_t));
  nenv.size = env->size-size;
  if (var_of_dim && size){
    var_t* var_of_dim2 = malloc(size*sizeof(var_t));
    memcpy(var_of_dim2,var_of_dim,size*sizeof(var_t));
    qsort(var_of_dim2,size,sizeof(var_t),var_cmp);
    j=0;
    for (i=0;i<nenv.size; i++){
      while (j<size){
	int sgn = environment_var->compare(env->var_of_dim[i+j],var_of_dim2[j]);
	assert(sgn<=0);
	if (sgn==0)
	  j++;
	else
	  break;
      }
      nenv.var_of_dim[i] = environment_var->copy(env->var_of_dim[i+j]);
    }
    while (j<size){
      int sgn = environment_var->compare(env->var_of_dim[nenv.size+j],var_of_dim2[j]);
      if (sgn==0)
	  j++;
      else {
	env_clear(&nenv);
	nenv.size = UINT_MAX;
	break;
      }
    }
    free(var_of_dim2);
  }
  return nenv;
}

/* Least common environment of the two environments */
env_t env_lce(const env_t* e1, const env_t* e2)
{
  env_t e;
  size_t i,i1,i2;
  int sgn;

  e.var_of_dim = malloc((e1->size+e2->size)*sizeof(var_t));
  i = 0;
  i1 = 0;
  i2 = 0;
  while (i1<e1->size && i2<e2->size){
    sgn = environment_var->compare(e1->var_of_dim[i1],e2->var_of_dim[i2]);
    e.var_of_dim[i] = environment_var->copy(sgn>0 ? e2->var_of_dim[i2] : e1->var_of_dim[i1]);
    i++;
    if (sgn>0)
      i2++;
    else if (sgn<0)
      i1++;
    else {
      i1++; i2++;
    }
  }
  while (i1<e1->size){
    e.var_of_dim[i] = environment_var->copy(e1->var_of_dim[i1]);
    i++; i1++;
  }
  while (i2<e2->size){
    e.var_of_dim[i] = environment_var->copy(e2->var_of_dim[i2]);
    i++; i2++;
  }

  e.size = i;
  e.var_of_dim = realloc(e.var_of_dim, e.size*sizeof(var_t));
  return e;
}

/*
  - env1 is supposed to be subenvironment of env

  - The function computes in the already allocated dimchange1 the conversion
    permutation from env1 to env, using offsetwriter and offsetread to shift the
    dimensions.

  - return true iff wrong arguments
*/

bool env_lce_dimchange(const env_t* e1, const env_t* e,
		       dimchange_t* dimchange1,
		       size_t offsetwrite, size_t offsetread)
{
  size_t i,i1,k1;
  int sgn=0;

  if (e1->size==e->size) return false;
  if (e1->size>e->size) return true;
  i = 0;
  k1 = 0;
  for (i1=0; i1<e1->size; i1++){
    while (i<e->size){
      sgn = environment_var->compare(e1->var_of_dim[i1],e->var_of_dim[i]);
      i++;
      if (sgn<0) return true;
      if (sgn==0) break;
      /* here, a new dimension to insert */
      dimchange1->dim[offsetwrite+k1] = offsetread+i1;
      k1++;
    }
    if(sgn!=0) return true;
  }
  while (i<e->size){
    dimchange1->dim[offsetwrite+k1] = offsetread+i1;
    k1++;
  }
  return false;
}

/* Least common environment of an array of environments */
env_t env_lce_array(const env_t** tenv,
		    size_t* tindex,
		    size_t size)
{
  int sgn;
  size_t i,index,maxvars;
  var_t name;
  env_t env;

  maxvars=0;
  for (i=0; i<size; i++){
    maxvars += tenv[i]->size;
    tindex[i] = 0;
  }
  env.var_of_dim = malloc( maxvars*sizeof(var_t) );
  index = 0;
  while (true){
    /* Find one possible next variable, and exit if none */
    name = NULL;
    for (i=0;i<size;i++){
      if (tindex[i]<tenv[i]->size){
	name = tenv[i]->var_of_dim[tindex[i]];
	break;
      }
    }
    if (name==NULL) break;
    /* Find the minimal variable */
    for (i=1; i<size; i++){
      if (tindex[i]<tenv[i]->size){
	sgn = environment_var->compare(name,tenv[i]->var_of_dim[tindex[i]]);
	if (sgn>0)
	  name = tenv[i]->var_of_dim[tindex[i]];
      }
    }
    env.var_of_dim[index] = environment_var->copy(name);
    index++;
    /* Increment the indexes */
    for (i=0; i<size; i++){
      if (tindex[i]<tenv[i]->size){
	sgn = environment_var->compare(name,tenv[i]->var_of_dim[tindex[i]]);
	if (sgn==0)
	  tindex[i]++;
      }
    }
  }
  env.size = index;
  env.var_of_dim = realloc(env.var_of_dim, env.size*sizeof(var_t));
  return env;
}

/* ************************************************************************* */
/* II. environment_t */
/* ************************************************************************* */

/* ========================================================================= */
/* Datatype */
/* ========================================================================= */

/* The denv_t internal type store the binding name to dimension for all types */

typedef struct denv_t {
  env_t envint;
  env_t envreal;
} denv_t;


const denv_t denv_of_environment(const environment_t* env)
{
  denv_t res;
  res.envint = env_cons(env->var_of_dim,env->intdim);
  res.envreal = env_cons(env->var_of_dim ?
			 &env->var_of_dim[env->intdim] :
			 NULL,
			 env->realdim);
  return res;
}

environment_t* environment_of_denv(denv_t* denv)
{
  environment_t* res;
  res = malloc(sizeof(environment_t));
  res->var_of_dim = malloc((denv->envint.size + denv->envreal.size)*
			     sizeof(var_t));
  memcpy(&res->var_of_dim[0],
	 denv->envint.var_of_dim,denv->envint.size*sizeof(var_t));
  memcpy(&res->var_of_dim[denv->envint.size],
	 denv->envreal.var_of_dim,denv->envreal.size*sizeof(var_t));
  free(denv->envint.var_of_dim);
  free(denv->envreal.var_of_dim);
  res->intdim = denv->envint.size;
  res->realdim = denv->envreal.size;
  res->count = 1;
  return res;
}

/* ========================================================================= */
/* Access */
/* ========================================================================= */
dim_t environment_dim_of_var(const environment_t* env, var_t name){
  var_t* res;
  res = bsearch(&name,env->var_of_dim,env->intdim,sizeof(var_t),var_cmp);
  if (res!=NULL){
    return ((long int)res - (long int)env->var_of_dim)/sizeof(var_t);
  }
  else {
    res = bsearch(&name,&env->var_of_dim[env->intdim],env->realdim,sizeof(var_t),var_cmp);
    if (res!=NULL){
      return ((long int)res - (long int)env->var_of_dim)/sizeof(var_t);
    }
    else
      return DIM_MAX;
  }
}

/* ========================================================================= */
/* Checks */
/* ========================================================================= */


bool environment_check(const environment_t* env)
{
  const denv_t denv = denv_of_environment(env);

  /* Check that there is no doublons */
  if (env_check(&denv.envint)) return true;
  if (env_check(&denv.envreal)) return true;
  /* Check that integer and real variables are disjoint */
  if (! env_is_inter_empty(&denv.envint,&denv.envreal)) return true;
  return false;
}

bool environment_check_compatibility(const environment_t* env1,
				     const environment_t* env2)
{
  const denv_t denv1 = denv_of_environment(env1);
  const denv_t denv2 = denv_of_environment(env2);
  /* Check that integer of one and real variables of other are disjoint */
  return !(env_is_inter_empty(&denv1.envint,&denv2.envreal) &&
	   env_is_inter_empty(&denv2.envint,&denv1.envreal) );
}

/* ========================================================================= */
/* Memory management */
/* ========================================================================= */

environment_t* environment_add(const environment_t* env,
			       var_t* name_of_intdim, size_t intdim,
			       var_t* name_of_realdim, size_t realdim)
{
  environment_t* res;
  denv_t denv2;

  const denv_t denv1 = denv_of_environment(env);

  denv2.envint = env_add(&denv1.envint, name_of_intdim, intdim);
  denv2.envreal = env_add(&denv1.envreal, name_of_realdim, realdim);
  res = environment_of_denv(&denv2);
  /* Check now that there is no problem */
  if (environment_check(res)){
    environment_free(res);
    return NULL;
  }
  return res;
}

environment_t* environment_add_perm(const environment_t* env,
				    var_t* name_of_intdim, size_t intdim,
				    var_t* name_of_realdim, size_t realdim,
				    dim_t** permutation)
{
  dim_t* perm;
  environment_t* res;
  size_t i,nsize;
  denv_t denv2;
  const denv_t denv1 = denv_of_environment(env);

  nsize = env->intdim+intdim+env->realdim+realdim:
  perm = malloc(nsize*sizeof(dim_t));
  for (i=0;i<nsize;i++) 
    perm[i]=i;
  denv2.envint = env_add_perm(&denv1.envint, name_of_intdim, intdim,
			      perm);
  denv2.envreal = env_add(&denv1.envreal, name_of_realdim, realdim,
			  perm + env->intdim+intdim);
  res = environment_of_denv(&denv2);
  /* Check now that there is no problem */
  if (environment_check(res)){
    environment_free(res);
    free(perm);
    *permutation = NULL;
    return NULL;
  }
  *permutation = perm;
  return res;
}

environment_t* environment_remove(const environment_t* env,
				  var_t* name_of_intdim, size_t intdim,
				  var_t* name_of_realdim, size_t realdim)
{
  environment_t* res;
  denv_t denv2;

  const denv_t denv1 = denv_of_environment(env);

  denv2.envint = env_remove(&denv1.envint, name_of_intdim, intdim);
  denv2.envreal = env_remove(&denv1.envreal, name_of_realdim, realdim);
  if (denv2.envint.size==UINT_MAX ||
      denv2.envreal.size==UINT_MAX){
    res = NULL;
  } else {
    res = environment_of_denv(&denv2);
  }
  return res;
}
environment_t* environment_make(var_t* name_of_intdim, size_t intdim,
			       var_t* name_of_realdim, size_t realdim)
{
  const environment_t env = { NULL, 0,0,0 };
  return environment_add(&env,
			 name_of_intdim, intdim,
			 name_of_realdim, realdim);
}
environment_t* environment_make_empty()
{
  return environment_make(NULL,0,NULL,0);
}

void environment_free2(environment_t* env)
{
  size_t i;
  if (env->var_of_dim){
    for(i=0;i<env->intdim+env->realdim;i++){
      if(env->var_of_dim[i]){
	environment_var->free(env->var_of_dim[i]);env->var_of_dim[i]=NULL;
      }
    }
    free(env->var_of_dim); env->var_of_dim = NULL;
  }
  env->count = 0;
  free(env);
}
void environment_fdump(FILE* stream, environment_t* env)
{
  size_t i;
  fprintf(stream,"environment: dim = (%u,%u), count = %u\n",
	  env->intdim,env->realdim,env->count);
  for (i=0; i<env->intdim+env->realdim; i++){
    fprintf(stream,"%2u: ",i);
    environment_var->fprint(stream,env->var_of_dim[i]);
    fprintf(stream,"\n");
  }
}

environment_name_of_dim_t* environment_name_of_dim_alloc(environment_t* env)
{
  size_t i,size;
  size = env->intdim+env->realdim;
  environment_name_of_dim_t* res = malloc(sizeof(environment_name_of_dim_t)+size*sizeof(char*));
  res->size = size;
  for (i=0; i<size; i++){
    res->p[i] = environment_var->to_string(env->var_of_dim[i]);
  }
  return res;
}
void environment_name_of_dim_free(environment_name_of_dim_t* p)
{
  size_t i;
  for (i=0; i<p->size; i++){
    free(p->p[i]);
  }
  free(p);
}

/* ========================================================================= */
/* Tests */
/* ========================================================================= */

bool environment_is_eq(const environment_t* env1,
		       const environment_t* env2)
{
  bool res = (env1==env2);
  if (!res){
    res =
      (env1->intdim==env2->intdim) &&
      (env1->realdim==env2->realdim);
    if (res){
      size_t i;
      for (i=0; i<env1->intdim+env1->realdim; i++){
	if (environment_var->compare(env1->var_of_dim[i],env2->var_of_dim[i])){
	  res = false;
	  break;
	}
      }
    }
  }
  return res;
}

bool environment_is_leq(const environment_t* env1,
		       const environment_t* env2)
{
  bool res = (env1==env2);
  if (!res){
    res =
      (env1->intdim<=env2->intdim) &&
      (env1->realdim<=env2->realdim);
    if (res){
      const denv_t denv1 = denv_of_environment(env1);
      const denv_t denv2 = denv_of_environment(env2);
      res =
	env_is_leq(&denv1.envint,&denv2.envint) &&
	env_is_leq(&denv1.envreal,&denv2.envreal);
      }
  }
  return res;
}

int environment_compare(const environment_t* env1,
			const environment_t* env2)
{
  dimchange_t* dimchange1;
  dimchange_t* dimchange2;
  environment_t* e;
  int res;

  e = environment_lce(env1,env2,&dimchange1,&dimchange2);
  if (e==NULL) res = -2;
  else if (dimchange1==NULL && dimchange2==NULL) res = 0;
  else if (dimchange1==NULL) res = 1;
  else if (dimchange2==NULL) res = -1;
  else res = 2;
  if (dimchange1 && dimchange2) environment_free(e);
  if (dimchange1) dimchange_free(dimchange1);
  if (dimchange2) dimchange_free(dimchange2);
  return res;
}

/* ========================================================================= */
/* Compute least common environment of 2 environments */
/* ========================================================================= */

dimchange_t* environment_dimchange(const environment_t* env1,
				   const environment_t* env)
{
  bool b;
  dimchange_t* dimchange;

  const denv_t denv1 = denv_of_environment(env1);
  const denv_t denv = denv_of_environment(env);

  dimchange = dimchange_alloc(env->intdim - env1->intdim,
			      env->realdim - env1->realdim);
  b = env_lce_dimchange(&denv1.envint,&denv.envint,
			dimchange, 0, 0);
  if (!b)
    b = env_lce_dimchange(&denv1.envreal,&denv.envreal,
			  dimchange, dimchange->intdim, env1->intdim);
  if (b){
    dimchange_free(dimchange);
    dimchange = NULL;
  }
  return dimchange;
}

/*
  Least common environment to two environment.

   - Assume environment_is_eq(env1,env2)==false.

   - If environments are not compatible, return NULL

   - Compute also in dimchange1 and dimchange2 the conversion transformations.

   - If no dimensions to add to env1, this implies that env is
     actually env1. In this case, *dimchange1==NULL.
 */
environment_t* environment_lce(const environment_t* env1,
			       const environment_t* env2,
			       dimchange_t** dimchange1,
			       dimchange_t** dimchange2)
{
  size_t size;
  denv_t denv;

  if (environment_check_compatibility(env1,env2))
    return NULL;

  const denv_t denv1 = denv_of_environment(env1);
  const denv_t denv2 = denv_of_environment(env2);

  denv.envint = env_lce(&denv1.envint, &denv2.envint);
  denv.envreal = env_lce(&denv1.envreal, &denv2.envreal);
  size = denv.envint.size+denv.envreal.size;

  if (denv.envint.size==denv1.envint.size &&
      denv.envreal.size==denv1.envreal.size){
    /* env=env1 */
    *dimchange1 = NULL;
    *dimchange2 = dimchange_alloc(denv.envint.size-denv2.envint.size,
				  denv.envreal.size-denv2.envreal.size);
    env_lce_dimchange(&denv2.envint,&denv.envint,
		      *dimchange2, 0,0);
    env_lce_dimchange(&denv2.envreal,&denv.envreal,
		      *dimchange2, (*dimchange2)->intdim, denv2.envint.size);
    env_clear(&denv.envint);
    env_clear(&denv.envreal);
    return (environment_t*)env1;
  }
  else if (denv.envint.size==denv2.envint.size &&
	   denv.envreal.size==denv2.envreal.size){
    /* env=env2 */
    *dimchange2 = NULL;
    *dimchange1 = dimchange_alloc(denv.envint.size-denv1.envint.size,
				  denv.envreal.size-denv1.envreal.size);
    env_lce_dimchange(&denv1.envint,&denv.envint,
		      *dimchange1, 0, 0);
    env_lce_dimchange(&denv1.envreal,&denv.envreal,
		      *dimchange1, (*dimchange1)->intdim, denv1.envint.size);
    env_clear(&denv.envint);
    env_clear(&denv.envreal);
    return (environment_t*)env2;
  }
  else {
    *dimchange1 = dimchange_alloc(denv.envint.size-denv1.envint.size,
				  denv.envreal.size-denv1.envreal.size);
    env_lce_dimchange(&denv1.envint,&denv.envint,
		      *dimchange1, 0, 0);
    env_lce_dimchange(&denv1.envreal,&denv.envreal,
		      *dimchange1, (*dimchange1)->intdim, denv1.envint.size);
    *dimchange2 = dimchange_alloc(denv.envint.size-denv2.envint.size,
				  denv.envreal.size-denv2.envreal.size);
    env_lce_dimchange(&denv2.envint,&denv.envint,
		      *dimchange2, 0,0);
    env_lce_dimchange(&denv2.envreal,&denv.envreal,
		      *dimchange2, (*dimchange2)->intdim, denv2.envint.size);
    return environment_of_denv(&denv);
  }
}

/* ========================================================================= */
/* Compute least common environment of an array of environments */
/* ========================================================================= */

/*
  Least common environment to an array environments.

  - Assume the size of the array is at least one.

  - If all input environments are the same, *ptdimchange==NULL.
    Otherwise, compute in *ptdimchange the conversion permutations.

  - Dimensions to add before applying the permutation (according to the
    conventions of the function abstract0_add_permute_dimensions) are implictly
    given by env->intdim-env[i]->intdim and env->realdim - env[i]->realdim.

  - If no dimensions to add to env[i], this implies that env is actually
    env[i]. In this case, (*ptdimchange)[i]==NULL. Otherwise, the function allocates
    the array (*ptdimchange)[0] with malloc.

 */
static int env_cmp(const void* a, const void* b)
{
  environment_t* pa = *((environment_t**)a);
  environment_t* pb = *((environment_t**)b);
  return (pa>pb ? 1 : (pa==pb ? 0 : -1));
}

environment_t* environment_lce_array(const environment_t** tenv,
				     size_t size,
				     dimchange_t*** ptdimchange)
{
  size_t i, intrealdim;
  denv_t* tdenv;
  const env_t** te;
  size_t* tindex;
  denv_t denv;
  int j;
  bool change;

  assert(size>=1);

  qsort(tenv,size,sizeof(environment_t*),env_cmp);
  /* remove doublons */
  for(i=0;i<size-1;i++){
    if (tenv[i]==tenv[i+1]){
      if (i<size-2){
	memmove(&tenv[i+1],&tenv[i+2],(size-i-2)*sizeof(environment_t*));
      }
      size--;
    }
  }
  if (size==1){
    *ptdimchange = malloc(sizeof(dim_t*));
    (*ptdimchange)[0] = NULL;
    return (environment_t*)tenv[0];
  }
  if (size==2){
    if (environment_is_eq(tenv[0],tenv[1])){
      *ptdimchange = NULL;
      return (environment_t*)tenv[0];
    }
    else {
      *ptdimchange = malloc(2*sizeof(dimchange_t*));
      return environment_lce(tenv[0],tenv[1],&(*ptdimchange)[0],&(*ptdimchange)[1]);
    }
  }

  /* Compute env */
  tdenv = malloc(size*sizeof(denv_t));
  for (i=0;i<size;i++){
    (denv_t)tdenv[i] = denv_of_environment(tenv[i]);
  }

  tindex = malloc(size*sizeof(size_t));
  te = malloc(size*sizeof(env_t*));

  for (i=0;i<size;i++){
    te[i] = &tdenv[i].envint;
  }
  denv.envint = env_lce_array(te,tindex,size);
  for (i=0;i<size;i++){
    te[i] = &tdenv[i].envreal;
  }
  denv.envreal = env_lce_array(te,tindex,size);
  free(te);
  free(tindex);

  intrealdim = denv.envint.size + denv.envreal.size;

  /* Compute transformations */
  *ptdimchange = malloc(size*sizeof(dimchange_t*));
  change = false;
  j = -1;
  for (i=0;i<size;i++){
    if (tdenv[i].envint.size==denv.envint.size &&
	tdenv[i].envreal.size==denv.envreal.size){
      (*ptdimchange)[i] = NULL;
      if (j<0) j = (int)i;
    }
    else {
      change = true;
      (*ptdimchange)[i] = dimchange_alloc(denv.envint.size-tdenv[i].envint.size,
					  denv.envreal.size-tdenv[i].envreal.size);
      env_lce_dimchange(&tdenv[i].envint,&denv.envint,
			(*ptdimchange)[i],0,0);
      env_lce_dimchange(&tdenv[i].envreal,&denv.envreal,
			(*ptdimchange)[i],(*ptdimchange)[i]->intdim, tdenv[i].envint.size);
    }
  }
  if (j>0){
    env_clear(&denv.envint);
    env_clear(&denv.envreal);
    if (!change){
      free(*ptdimchange);
      *ptdimchange=NULL;
    }
    return environment_copy((environment_t*)tenv[j]);
  }
  else {
    environment_t* env = environment_of_denv(&denv);
    if (environment_check(env)){
      for (i=0;i<size;i++){
	if ((*ptdimchange)[i]) free((*ptdimchange)[i]);
      }
      free(*ptdimchange);
      *ptdimchange=NULL;
      environment_free(env);
      env = NULL;
    }
    return env;
  }
}

/* ========================================================================= */
/* Renaming the variables of an environment */
/* ========================================================================= */

environment_t* environment_rename(environment_t* env,
				  var_t* tvar1, var_t* tvar2, size_t size,
				  dim_t** permutation)
{
  size_t i,j,nbdims;
  dim_t* perm;
  environment_t* res;
  denv_t denv;

  *permutation = NULL;
  nbdims = env->intdim+env->realdim;
  res = malloc(sizeof(environment_t));
  res->intdim = env->intdim;
  res->realdim = env->realdim;
  res->count = 1;
  res->var_of_dim = malloc(nbdims*sizeof(var_t));

  /* Build the new environment */
  for (i=0; i<nbdims; i++){
    res->var_of_dim[i] = NULL;
  }
  for (j=0; j<size; j++){
    dim_t dim = environment_dim_of_var(env,tvar1[j]);
    if (dim==DIM_MAX){
      environment_free(res);
      return NULL;
    }
    res->var_of_dim[dim] = environment_var->copy(tvar2[j]);
  }
  for (i=0; i<nbdims; i++){
    if (res->var_of_dim[i] == NULL){
      res->var_of_dim[i] = environment_var->copy(env->var_of_dim[i]);
    }
  }

  /* Now we have to sort it and compute the permutation transformation */
  perm = malloc(nbdims*sizeof(dim_t));
  for (i=0;i<nbdims;i++)
    perm[i] = i;
  denv = denv_of_environment(res);
  env_normalize_perm(&denv.envint,perm);
  env_normalize_perm(&denv.envreal,perm+res->intdim);
  *permutation = perm;
  if (environment_check(res)){
    environment_free(res);
    free(perm);
    *permutation = NULL;
    return NULL;
  }
  return res;
}
